...
if __name__ == "__main__":
    ...
    try:
        ...
        param_dict = load_params("./Features/disease")
        mlpnn_scores = {}
        for l in label_set:
            mlpnn_scores[l] = pd.read_csv(path + "/feature_evaluation/mlpnn_feature_scores_" + str(l) + ".csv", index_col=0, header=0)
            # mlpnn_scores[l] = pd.read_csv(path + "/feature_evaluation/mlpnn_feature_scores_D.csv",
            #                               index_col=0, header=0)
            # mlpnn_scores[l] = pd.read_csv(path + "/feature_evaluation/mlpnn_feature_scores_H.csv",
            #                               index_col=0, header=0)
        train_mlpnn = "True"
    ...
    results_sub_df = pd.DataFrame(index=["AUC", "MCC", "Precision", "Recall", "F1","ACC"], columns=["RF", "SVM", "Logistic Regression", "MLPNN"])
    auc_sub_list = {}
    mcc_sub_list = {}
    precision_sub_list = {}
    recall_sub_list = {}
    ...
    if train_rf == "True":
        ...
        acc_sub_list["RF"].append(rf_stats["ACC"])
        ...
        results_sub_df.loc["ACC"]["RF"] = "{:.2f}".format(np.mean(rf_stats["ACC"]))
    ...
    if train_svm == "True":
        ...
        acc_sub_list["SVM"].append(svm_stats["ACC"])
        ...
        results_sub_df.loc["ACC"]["SVM"] = "{:.2f}".format(np.mean(svm_stats["ACC"]))
    ...
    if train_logistic_regression == "True":
        ...
        acc_sub_list["Logistic Regression"].append(lr_stats["ACC"])
        ...
        results_sub_df.loc["ACC"]["Logistic Regression"] = "{:.2f}".format(np.mean(lr_stats["ACC"]))
    
    if train_mlpnn == "True":
            
        mlpnn_model = MLPNN(train_x.shape[1], num_class, config, param_dict["MLPNN"])
        mlpnn_model.train(train_oh)            
        mlpnn_stats,mlpnn_probs,mlpnn_pred = mlpnn_model.test(train_oh)
        ...
        acc_sub_list["MLPNN"].append(mlpnn_stats["ACC"])
        ...
        results_sub_df.loc["ACC"]["MLPNN"] = "{:.2f}".format(np.mean(mlpnn_stats["ACC"]))

    results_sub_df.to_csv(path + "/final_models_" + str(num_feat) + "/training_results.tsv", sep="\t")
    
    if external_path != None:

        external_data_df = pd.read_csv("./" + external_path + "/abundance.tsv", index_col=0, header=None, sep="\t",encoding='unicode_escape').loc[np.intersect1d(abundance.index.values, features_subset.reshape(-1))]
        # print(external_data_df)
        # external_data_df = pd.read_csv("./" + external_path + "/abundance.tsv", index_col=0, header=None, sep="\t",encoding='unicode_escape')
        # print(external_data_df)
        # external_data_df = external_data_df.T
        external_labels_df = pd.read_csv("./" + external_path + "/labels.txt", index_col=0, header=None).index.values
        results_external_sub_df = pd.DataFrame(index=["AUC", "MCC", "Precision", "Recall", "F1","ACC"],
                                                       columns=["RF", "SVM", "Logistic Regression", "MLPNN","VOT"])
        # print(external_data_df.transpose().values.type)

        # values_= int(external_data_df.transpose().values) + int(1)


        # log_external_sub = np.log(float(external_data_df.transpose().values))
        #
        if normalization == "MinMax":
            test = np.clip(scaler.transform(external_data_df.transpose().values), 0, 1)
        if normalization == "Standard":
            test = np.clip(scaler.transform(external_data_df.transpose().values), -3, 3)
        external_labels = []
        ...
        # svm_clf = joblib.load('./Model_results/svm_model.pkl')
        # rf_clf = joblib.load('./Model_results/rf_model.pkl')
        # lr_clf = joblib.load('./Model_results/logistic_regression_model.pkl')
        # mlpnn_model = load_model('./Model_results/mlpnn.h5')

        if train_rf == "True":
            rf_pred = np.argmax(rf_clf.predict_proba(test), axis=1)
            rf_probs = rf_clf.predict_proba(test)
            rf_stats = get_stat_dict(external_labels, rf_probs, rf_pred)
            ...
            results_external_sub_df.loc["ACC"]["RF"] = "{:.2f}".format(rf_stats["ACC"])

        if train_svm == "True":
            svm_pred = np.argmax(svm_clf.predict_proba(test), axis=1)
            svm_probs = svm_clf.predict_proba(test)
            svm_stats = get_stat_dict(external_labels, svm_probs, svm_pred)
            ...
            results_external_sub_df.loc["ACC"]["SVM"] = "{:.2f}".format(svm_stats["ACC"])
            
        if train_logistic_regression == "True":
            lr_pred =  np.argmax(lr_clf.predict_proba(test), axis=1)
            lr_probs = lr_clf.predict_proba(test)
            lr_stats = get_stat_dict(external_labels, lr_probs, lr_pred)
            ...
            results_external_sub_df.loc["ACC"]["Logistic Regression"] = "{:.2f}".format(lr_stats["ACC"])
            
        if train_mlpnn == "True":
            mlpnn_stats,mlpnn_prob, mlpnn_pred= mlpnn_model.test((test, external_labels_oh))
            ...
            results_external_sub_df.loc["ACC"]["MLPNN"] = "{:.2f}".format(mlpnn_stats["ACC"])

        # mlpnn_model = load_model(path + "/final_models_" + str(num_feat) + "/mlpnn.h5")
        # mlpnn_model = KerasClassifier(build_fn=mlpnn_model)
        # mlpnn_model._estimator_type = "classifier"
        # vot_model = VotingClassifier(
        #     estimators=[('rf', rf_clf), ('svm', svm_clf), ("lr", lr_clf),("mlpnn",mlpnn_model.train)],voting='soft')
        # # joblib.dump(vot_model, 'vot_model.pkl')
        # vot_model.fit(train_x, labels)
        # pred = np.argmax(vot_model.predict_proba(test), axis=1)
        # probs = vot_model.predict_proba(test)
        # vot_stats = get_stat_dict(external_labels, probs, pred)
        # results_external_sub_df.loc["AUC"]["VOT"] = "{:.2f}".format(vot_stats["AUC"])
        # results_external_sub_df.loc["MCC"]["VOT"] = "{:.2f}".format(vot_stats["MCC"])
        # results_external_sub_df.loc["Precision"]["VOT"] = "{:.2f}".format(vot_stats["Precision"])
        # results_external_sub_df.loc["Recall"]["VOT"] = "{:.2f}".format(vot_stats["Recall"])
        # results_external_sub_df.loc["F1"]["VOT"] = "{:.2f}".format(vot_stats["F1"])
        # results_external_sub_df.loc["ACC"]["VOT"] = "{:.2f}".format(vot_stats["ACC"])

        # final_probs = (rf_probs+svm_probs + lr_probs + mlpnn_prob) / 4
        # final_probs = np.array(final_probs)
        # print(finalpred)

        final_pred = []
        for i in range(0, len(test)):
            if rf_pred[i]+svm_pred[i]+lr_pred[i]+mlpnn_pred[i]>=1:
               final_pred.append(1)
            if rf_pred[i]+svm_pred[i]+lr_pred[i]+mlpnn_pred[i]<1:
               final_pred.append(0)
        final_pred = np.array(final_pred)
        print(final_pred)
        from sklearn.metrics import roc_auc_score, accuracy_score, matthews_corrcoef, precision_score, recall_score, \
            f1_score

        results_external_sub_df.loc["MCC"]["VOT"] = "{:.2f}".format(matthews_corrcoef(external_labels,final_pred.reshape(-1)))
        # results_external_sub_df.loc["Precision"]["VOT"] = "{:.2f}".format(precision_score(external_labels,final_pred, average='weighted'))
        # results_external_sub_df.loc["Recall"]["VOT"] = "{:.2f}".format(recall_score(external_labels,final_pred,average='weighted'))
        # results_external_sub_df.loc["F1"]["VOT"] = "{:.2f}".format(f1_score(external_labels,final_pred, average='weighted'))
        results_external_sub_df.loc["Precision"]["VOT"] = "{:.2f}".format(precision_score(external_labels, final_pred))
        results_external_sub_df.loc["Recall"]["VOT"] = "{:.2f}".format(recall_score(external_labels, final_pred))
        results_external_sub_df.loc["F1"]["VOT"] = "{:.2f}".format(f1_score(external_labels, final_pred))
        results_external_sub_df.loc["ACC"]["VOT"] = "{:.2f}".format(accuracy_score(external_labels,final_pred))
