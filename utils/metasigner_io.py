...
def get_stat_dict(y, probs, pred):
    ...
    # stat_dict["ACC"] = accuracy_score(y, pred)
    if num_class == 2:
        stat_dict["AUC"] = roc_auc_score(y, probs[:,1])
    else:

        stat_dict["AUC"] = roc_auc_score(y_oh, probs)
    stat_dict["MCC"] = matthews_corrcoef(y, pred.reshape(-1))
    stat_dict["Precision"] = precision_score(y, pred)
    stat_dict["Recall"] = recall_score(y, pred)
    stat_dict["F1"] = f1_score(y, pred)
    stat_dict["ACC"] = accuracy_score(y, pred)
    C2 = confusion_matrix(y, pred, labels=[0, 1])
    print(C2)
    print(y,pred)
