...
if __name__ == "__main__":
    ...
    # Read in Config File
    #####################################################################
    path,k,result_path = MakeOption()
    config = get_config()
    datasets = path.split("/")[-1].strip()
    max_k = k
    ...
    for dataset in datasets:
        ...
        os.system("Rscript ./Meta-Signer-master/src/R/aggregate_rankings.R %s %s %s %s %s" % (result_path,max_k,"raw","ensemble",agg_method))
